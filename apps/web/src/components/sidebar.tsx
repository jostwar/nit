...
const ICONS: Record<string, () => React.JSX.Element> = {
  ...
  ai: IconAsk,
  dataUpdate: IconSync,
  adminUsers: IconUsers,
  adminFilters: IconSettings,
  adminCatalog: IconCatalog,
  adminReports: IconReports,
};
    ...
    <aside className="fixed left-0 top-0 z-30 flex h-screen w-56 flex-shrink-0 flex-col border-r border-slate-700/50 bg-[var(--sidebar-bg)] px-3 py-5 lg:w-60">
      ...
      <nav className="flex flex-col gap-0.5">
        {visibleNav.map((item) => {
          ...
          return (
            <Link key={item.href} href={hrefWithParams(item.href)} className={linkClass(item.href)}>
              {Icon && <Icon />}
              {item.label}
            </Link>
          );
      ...
      <div className="mx-3 mt-6 mb-3 border-t border-slate-600/50" />
      ...
      <div className="mt-auto px-3 pt-4 text-center">
        <p className="text-[10px] font-medium text-slate-400">BPI - Soluciones Empresariales</p>
        <p className="text-[9px] text-slate-500">Powered By iPeakAgency</p>
      </div>
    </aside>
  );
}
